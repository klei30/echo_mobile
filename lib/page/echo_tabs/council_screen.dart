...
class CouncilScreen extends StatefulWidget {
  ...
  const CouncilScreen({super.key, this.initialQuestion, this.threadId, this.threadContext, this.showChrome = true});
...
class _CouncilScreenState extends State<CouncilScreen> with TickerProviderStateMixin {
  ...
  Future<void> _ask() async {
    ...
    final result = await EchoApiClient().askCouncil(q, threadId: widget.threadId, threadContext: widget.threadContext);
    if (!mounted) return;
  ...
  Future<void> _saveDecisionOutcome({required bool proof}) async {
    final verdict = _result?['verdict'] as String? ?? '';
    final question = _result?['question'] as String? ?? _ctrl.text;
    await OutcomeCaptureSheet.show(
      context,
      title: proof ? 'Save this decision as proof?' : 'Save this decision outcome?',
      subjectType: 'decision_room',
      subjectId: widget.threadId,
      contextNote: 'Question: $question\nTakeaway: $verdict',
      doneLabel: proof ? 'Save proof' : 'Saved action',
      skippedLabel: 'Not useful',
      createProof: proof,
      proofCategory: proof ? 'decision' : 'outcome',
      proofTitle: question.isEmpty ? 'Decision Room takeaway' : question,
          ...
          children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(widget.threadContext!, style: GoogleFonts.plusJakartaSans(color: EchoColors.textMuted, fontSize: 13, height: 1.5)),
                  ],
                ),
              ),
            ],
            Text(
              'Bring a decision to several perspectives.',
              style: GoogleFonts.plusJakartaSans(color: EchoColors.textPrimary, fontSize: 20, fontWeight: FontWeight.w600),
            ),
            const SizedBox(height: 8),
            Text(
              'Five distinct perspectives. They will disagree. That\'s the point.',
              style: GoogleFonts.plusJakartaSans(color: EchoColors.textMuted, fontSize: 13, height: 1.5),
            ),
            ...
            Container(
              ...
              child: TextField(
                ...
                maxLines: 4,
                minLines: 2,
                style: GoogleFonts.plusJakartaSans(color: EchoColors.textPrimary, fontSize: 15, height: 1.6),
                decoration: InputDecoration(
                  hintText: 'Should I accept this offer?\nWhat do I do about this situation?',
                  hintStyle: GoogleFonts.plusJakartaSans(color: EchoColors.textMuted.withValues(alpha: 0.5), fontSize: 14, height: 1.6),
            ...
            SizedBox(
              ...
              child: GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(colors: [EchoColors.amber, EchoColors.amber.withValues(alpha: 0.7)]),
                    borderRadius: BorderRadius.circular(14),
                    boxShadow: [BoxShadow(color: EchoColors.amber.withValues(alpha: 0.28), blurRadius: 18, spreadRadius: 0)],
                  ),
                  child: Center(
                    child: Text(
                      'Run perspectives',
                      style: GoogleFonts.plusJakartaSans(color: Colors.black, fontSize: 15, fontWeight: FontWeight.w700, letterSpacing: 0.3),
                    ),
                  ),
                ),
              ),
                ...
                return Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      Text(
                        name,
                        style: GoogleFonts.plusJakartaSans(color: color.withValues(alpha: 0.9), fontSize: 11, fontWeight: FontWeight.w500),
                      ),
                    ],
                  ),
                );
    ...
    return Center(
      child: Column(
        ...
        children: [
            ...
            builder: (context, child) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: EchoColors.amber.withValues(alpha: 0.25 + t * 0.15), width: 1.5),
                  boxShadow: [
                    BoxShadow(
          ...
          Text('Perspectives are working...', style: GoogleFonts.plusJakartaSans(color: EchoColors.textMuted, fontSize: 14, letterSpacing: 0.3)),
          const SizedBox(height: 8),
          Text('Several angles. One question.', style: GoogleFonts.plusJakartaSans(color: EchoColors.textMuted.withValues(alpha: 0.5), fontSize: 12)),
        ],
      ),
    );
  }
      ...
      child: ListView(
        ...
        children: [
          ...
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
            margin: const EdgeInsets.only(bottom: 20),
            decoration: BoxDecoration(color: EchoColors.bgSurface, borderRadius: BorderRadius.circular(10)),
            child: Text(
              '"$question"',
              style: GoogleFonts.plusJakartaSans(color: EchoColors.textMuted, fontSize: 13, fontStyle: FontStyle.italic, height: 1.5),
            ),
          ),
              ...
              child: Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      verdict,
                      style: GoogleFonts.plusJakartaSans(color: EchoColors.textPrimary, fontSize: 14, height: 1.55, fontWeight: FontWeight.w500),
                    ),
                  ],
                ),
              ),
          ...
          Row(
            children: [
              Expanded(child: _resultActionButton('Save outcome', Icons.check_circle_outline_rounded, () => _saveDecisionOutcome(proof: false))),
              const SizedBox(width: 10),
              Expanded(child: _resultActionButton('Add proof', Icons.inventory_2_outlined, () => _saveDecisionOutcome(proof: true))),
            ],
          ),
          ...
          if (widget.threadId != null) ...[
            GestureDetector(
              onTap: () async {
                ...
                await EchoApiClient().resolveThread(widget.threadId!, note: 'perspectives heard');
  ...
  Widget _resultActionButton(String label, IconData icon, VoidCallback onTap) {
  ...
  Widget _buildVoiceCard(int index, String name, String text) {
    ...
    return FadeTransition(
      opacity: ctrl,
      child: SlideTransition(
        position: Tween<Offset>(begin: const Offset(0, 0.15), end: Offset.zero).animate(CurvedAnimation(parent: ctrl, curve: Curves.easeOut)),
        child: Container(
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        const SizedBox(width: 6),
                        Text('- $subtitle', style: GoogleFonts.plusJakartaSans(color: EchoColors.textMuted.withValues(alpha: 0.45), fontSize: 11)),
                      ],
                    ),
                    const SizedBox(height: 5),
                    Text(
                      text.isNotEmpty ? text : '-',
                      style: GoogleFonts.plusJakartaSans(color: EchoColors.textPrimary.withValues(alpha: 0.88), fontSize: 13, height: 1.55),
                    ),
                  ],
                ),
              ),
