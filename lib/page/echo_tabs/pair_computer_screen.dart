...
class _PairComputerScreenState extends State<PairComputerScreen> {
  ...
  Future<void> _connect(String raw) async {
    ...
    // delays — DNS on the emulator uses Google's resolvers which lag behind
    ...
    if (defaultTargetPlatform == TargetPlatform.android) {
      try {
        ...
        // Not an emulator or Echo not running on host — fall through to tunnel.
      }
    }

    // Retry up to 15 times (60s) — Cloudflare DNS can take up to ~60s on some resolvers
    Exception? lastErr;
    for (int attempt = 0; attempt < 15; attempt++) {
      ...
      try {
        ...
        // Only retry on DNS / socket errors — not on HTTP errors
