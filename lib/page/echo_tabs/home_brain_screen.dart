...
class _HomeBrainScreenState extends State<HomeBrainScreen> {
  ...
  void initState() {
    super.initState();
    // Quick tunnels die when the process stops â€” don't show a stale saved URL.
    _tunnelUrl = EchoHostService().hasTunnel ? EchoHostService().tunnelUrl : null;
    _pollHealth();
  ...
  // HomeBrainScreen always polls localhost directly â€” never via tunnel.
  ...
  Future<void> _pollHealth() async {
    // Public ping â€” no auth needed
    bool echoUp = false;
    try {
      final resp = await http.get(Uri.parse('$_localBase/health')).timeout(const Duration(seconds: 3));
    ...
    // Full system health â€” requires auth, always hit localhost
    setState(() => _echoRunning = true);
    try {
      final resp = await http.get(Uri.parse('$_localBase/v1/system/health'), headers: AuthService().authHeaders).timeout(const Duration(seconds: 5));
      if (!mounted) return;
  ...
  Future<void> _startEchoApi() async {
    ...
    // Use detached mode â€” cmd /k keeps the window open forever, so Process.run
    await Process.start('powershell.exe', [
      '-ExecutionPolicy',
      'Bypass',
      '-File',
      r'C:\Users\ASUS\Desktop\echo\start_echo_services.ps1',
      '-SkipGemma',
      '-SkipVoiceAgent',
  ...
  Future<void> _startTunnel() async {
    ...
    // Resolve cloudflared path â€” winget installs it to the Links folder
    final candidates = [
      'cloudflared',
    ...
    try {
      final process = await Process.start(cloudflaredExe, ['tunnel', '--url', 'http://localhost:8002']);
  ...
  Future<void> _waitForDns(String url) async {
    // Poll the actual tunnel /health endpoint â€” more reliable than DNS lookup alone
    ...
    for (int i = 0; i < 20; i++) {
      ...
      try {
        final resp = await http.get(Uri.parse('$url/health')).timeout(const Duration(seconds: 5));
        if (resp.statusCode == 200) {
          if (!mounted || !_tunnelDnsWarming) return;
          setState(() {
            _tunnelUrl = url;
    ...
    // 60s timeout â€” show QR anyway and let mobile retry handle it
  ...
  String _qrPayload(String url) => url;
      ...
      body: SafeArea(
        child: ListView(
          ...
          children: [
            ...
            Text(
              'Run Echo privately on this computer, use your home model and memory, and pair your phone by Wi-Fi or secure tunnel.',
              style: GoogleFonts.plusJakartaSans(fontSize: 12.5, color: EchoColors.textGhost, height: 1.5),
            ),
            const SizedBox(height: 20),

            _serviceRow(label: 'Echo service', running: _echoRunning, icon: Icons.api_rounded, starting: _startingEcho, onStart: _startEchoApi),
            ...
            _serviceRow(
              ...
              onStart: _startVllm,
              extra: _adapterLoaded == true ? 'personal style live' : null,
            ),
            const SizedBox(height: 8),
            _tunnelRow(),
            const SizedBox(height: 20),

            if (_tunnelUrl != null) ...[_qrCard(_tunnelUrl!), const SizedBox(height: 20)],
            ...
            if (_tunnelLog.isNotEmpty) ...[
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: _tunnelLog
                      .map((l) => Text(l, style: GoogleFonts.plusJakartaSans(fontSize: 10, color: EchoColors.textGhost, height: 1.5)))
                      .toList(),
                ),
              ),
            ],
  ...
  Widget _serviceRow({
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: isRunning ? const Color(0xFF4CAF50).withValues(alpha: 0.2) : EchoColors.borderSubtle),
      ),
      child: Row(
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(statusText, style: GoogleFonts.plusJakartaSans(fontSize: 11, color: statusColor)),
              ],
            ),
          ),
          if (!isRunning)
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: EchoColors.amber.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(8)),
                child: starting
                    ? SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 1.5, color: EchoColors.amber))
                    : Text(
                        'Start',
                        style: GoogleFonts.plusJakartaSans(fontSize: 11.5, fontWeight: FontWeight.w700, color: EchoColors.amberText),
                      ),
              ),
            ),
        ],
  ...
  Widget _tunnelRow() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: EchoColors.bgSurface,
        borderRadius: BorderRadius.circular(13),
        border: Border.all(color: active ? const Color(0xFF4CAF50).withValues(alpha: 0.2) : EchoColors.borderSubtle),
      ),
      child: Row(
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  style: GoogleFonts.plusJakartaSans(fontSize: 11, color: statusColor),
                ),
              ],
            ),
          ),
          GestureDetector(
            ...
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 6),
              decoration: BoxDecoration(
                color: active ? Colors.redAccent.withValues(alpha: 0.08) : EchoColors.amber.withValues(alpha: 0.1),
              ...
              child: _startingTunnel
                  ...
                  : Text(
                      ...
                      style: GoogleFonts.plusJakartaSans(
                        ...
                        color: active ? Colors.redAccent : EchoColors.amberText,
                      ),
                    ),
            ),
          ),
    ...
    return Container(
      ...
      child: Column(
        children: [
          Row(
            children: [
              Icon(Icons.phone_iphone_rounded, size: 16, color: EchoColors.amber),
              ...
              GestureDetector(
                ...
                child: Icon(Icons.copy_outlined, size: 15, color: EchoColors.textGhost),
              ),
            ],
          ),
          const SizedBox(height: 4),
          Text(
            'Open Echo on your phone -> Home Brain -> Scan QR. The code is the tunnel URL, so Android camera can read it too.',
            style: GoogleFonts.plusJakartaSans(fontSize: 11.5, color: EchoColors.textGhost, height: 1.5),
          ),
          ...
          Container(
            ...
            child: QrImageView(
              ...
              size: 180,
              eyeStyle: const QrEyeStyle(eyeShape: QrEyeShape.square, color: Color(0xFF0D0B08)),
              dataModuleStyle: const QrDataModuleStyle(dataModuleShape: QrDataModuleShape.square, color: Color(0xFF0D0B08)),
            ),
          ),
          ...
          Text(
            ...
            style: GoogleFonts.plusJakartaSans(fontSize: 10.5, color: EchoColors.textGhost, height: 1.5),
          ),
        ],
      ),
    );
