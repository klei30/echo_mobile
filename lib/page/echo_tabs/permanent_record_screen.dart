...
class _PermanentRecordScreenState extends State<PermanentRecordScreen> {
  ...
  Future<void> _load() async {
    ...
    final results = await Future.wait([EchoApiClient().getUserStats(), EchoApiClient().getConfidence()]);
    if (!mounted) return;
    final conf = results[1];
    setState(() {
      _stats = results[0];
      if (conf != null && conf['topics'] is List) {
        _topics = (conf['topics'] as List).map((t) => Map<String, dynamic>.from(t as Map)).toList();
      }
      _loading = false;
    });
  }
          ...
          child: ListView(
            ...
            children: [
              ...
              Padding(
                ...
                child: Text(
                  'LONG-TERM PROOF',
                  style: GoogleFonts.plusJakartaSans(fontSize: 9.5, fontWeight: FontWeight.w700, letterSpacing: 1.2, color: EchoColors.proof),
                ),
              ),
              Padding(
                    ...
                    : RichText(
                        text: TextSpan(
                          ...
                          children: [
                            TextSpan(
                              text: username,
                              style: TextStyle(color: EchoColors.textPrimary, fontWeight: FontWeight.w600),
                            ),
                            TextSpan(
                              text:
                                  ', observed for $weeksActive week${weeksActive == 1 ? '' : 's'}'
              ...
              if (_loading)
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: strong.map((t) {
                        ...
                        return _RecordRow(label: label, score: _pct(score), desc: _topicDesc(score), isStrong: true);
                      }).toList(),
                    ),
                  ),
                ],
                  ...
                  Padding(
                    ...
                    child: Column(
                      children: becoming.map((t) {
                        ...
                        final score = (t['score'] as num?)?.toDouble() ?? 0.0;
                        final label = _topicLabels[key] ?? key;
                        return _RecordRow(label: label, score: _pct(score), desc: _topicDesc(score), isStrong: false);
                      }).toList(),
                    ),
                  ),
                ],
                ...
                if (_topics.isEmpty)
                  Padding(
                    padding: const EdgeInsets.fromLTRB(20, 0, 20, 24),
                    child: Text(
                      'Keep talking - Echo is still building your archive.',
                      style: GoogleFonts.lora(fontSize: 14, fontStyle: FontStyle.italic, color: EchoColors.textGhost, height: 1.65),
                    ),
                  ),
                ...
                Padding(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      border: Border(left: BorderSide(color: EchoColors.amber.withValues(alpha: 0.5), width: 2)),
                    ),
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(
                          'ECHO\'S READ - FOR YOU ALONE',
                          style: GoogleFonts.plusJakartaSans(
                            fontSize: 9,
                            fontWeight: FontWeight.w700,
                            letterSpacing: 1.1,
                        ...
                        Text(
                          ...
                          'This is what you actually practice - not only what you said you would be.',
                          style: GoogleFonts.lora(fontSize: 13, fontStyle: FontStyle.italic, color: EchoColors.textMuted, height: 1.7),
                        ),
                      ],
                    ),
                  ),
...
class _RecordRow extends StatelessWidget {
  ...
  const _RecordRow({required this.label, required this.score, required this.desc, required this.isStrong});
          ...
          Container(
            ...
            decoration: BoxDecoration(
              gradient: LinearGradient(
                ...
                colors: [isStrong ? EchoColors.amber.withValues(alpha: 0.7) : const Color(0xFF5A6AB4).withValues(alpha: 0.5), Colors.transparent],
              ),
            ),
          ),
              ...
              children: [
                Row(
                  children: [
                    Text(
                      label,
                      style: GoogleFonts.plusJakartaSans(fontSize: 13.5, fontWeight: FontWeight.w500, color: EchoColors.textSecondary),
                    ),
                    const Spacer(),
                    Text(
                      score,
                      style: GoogleFonts.lora(fontSize: 13, fontStyle: FontStyle.italic, color: isStrong ? EchoColors.amber : EchoColors.textGhost),
                    ),
                  ],
                ),
                const SizedBox(height: 2),
                Text(desc, style: GoogleFonts.plusJakartaSans(fontSize: 11, color: EchoColors.textGhost, height: 1.5)),
              ],
            ),
          ),
        ],
