...
class ProofBuilderScreen extends StatefulWidget {
  final ProofBuilderIntent? initialIntent;
  final bool autoOpenDraft;

  const ProofBuilderScreen({super.key, this.initialIntent, this.autoOpenDraft = false});
...
class _ProofBuilderScreenState extends State<ProofBuilderScreen> {
  ...
  bool _loading = true;
  bool _saving = false;
  bool _openedInitialIntent = false;

  @override
  void initState() {
    super.initState();
    _load();
    if (widget.autoOpenDraft && widget.initialIntent != null) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _openInitialDraft());
    }
  }
  ...
  Future<void> _openInitialDraft() async {
    if (_openedInitialIntent || !mounted) return;
    _openedInitialIntent = true;
    await _addProof(intent: widget.initialIntent);
  }

  Future<void> _addProof({ProofBuilderIntent? intent}) async {
    final result = await showModalBottomSheet<_ProofDraft>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _ProofDraftSheet(intent: intent),
    );
    ...
    if (!mounted) return;
    EchoLoopReceipt.showFromDelta(context, Map<String, dynamic>.from(saved['loop_delta'] as Map? ?? {}), fallbackProofCreated: true);
  }
        ...
        child: RefreshIndicator(
          ...
          child: ListView(
            ...
            children: [
              ...
              Text(
                'Save artifacts, outcomes, practice wins, decisions, and feedback. Echo uses this to build stronger proof for jobs, school, scholarships, community projects, or personal goals.',
              ...
              if (widget.initialIntent != null) ...[const SizedBox(height: 16), _intentCard(widget.initialIntent!)],
              const SizedBox(height: 22),
              SizedBox(
                width: double.infinity,
                child: FilledButton.icon(
                  onPressed: _saving ? null : () => _addProof(intent: widget.initialIntent),
                  icon: _saving
          ...
          Row(
            ...
            children: [
              Container(
                ...
                child: Icon(Icons.inventory_2_outlined, size: 17, color: EchoColors.amber),
              ),
  ...
  Widget _intentCard(ProofBuilderIntent intent) {
    final label = intent.sourceLabel.isNotEmpty ? intent.sourceLabel : intent.opportunityType.replaceAll('_', ' ');
    final title = intent.title.isNotEmpty ? intent.title : 'Build the next missing proof';
    return Container(
      padding: const EdgeInsets.all(15),
      decoration: BoxDecoration(
        color: EchoColors.proof.withValues(alpha: 0.07),
        borderRadius: BorderRadius.circular(14),
      ...
      child: Row(
        ...
        children: [
          Container(
            width: 34,
            height: 34,
            decoration: BoxDecoration(color: EchoColors.proof.withValues(alpha: 0.10), borderRadius: BorderRadius.circular(10)),
            child: Icon(Icons.route_outlined, color: EchoColors.proof, size: 17),
          ),
          const SizedBox(width: 11),
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  label.toUpperCase(),
                  style: GoogleFonts.plusJakartaSans(fontSize: 9, letterSpacing: 1, fontWeight: FontWeight.w900, color: EchoColors.proof),
                ),
                const SizedBox(height: 4),
                Text(
                  title,
                  style: GoogleFonts.plusJakartaSans(fontSize: 13, height: 1.35, fontWeight: FontWeight.w800, color: EchoColors.textPrimary),
                ),
                if (intent.description.isNotEmpty) ...[
                  const SizedBox(height: 5),
                  Text(intent.description, style: GoogleFonts.plusJakartaSans(fontSize: 12, height: 1.45, color: EchoColors.textMuted)),
                ],
              ],
            ),
          ),
...
class _ProofDraftSheet extends StatefulWidget {
  final ProofBuilderIntent? intent;

  const _ProofDraftSheet({this.intent});
...
class _ProofDraftSheetState extends State<_ProofDraftSheet> {
  ...
  void initState() {
    ...
    _titleCtrl.text = intent.title;
    _descriptionCtrl.text = intent.description;
    _evidenceCtrl.text = intent.evidence;
    _tagsCtrl.text = intent.skillTags.join(', ');
    _category = intent.category;
    _opportunityType = intent.opportunityType;
  }
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.inventory_2_outlined, color: EchoColors.amber, size: 18),
                ...
                Text('Quick starts', style: _labelStyle()),
                const SizedBox(height: 8),
                Wrap(
                  spacing: 8,
                  runSpacing: 8,
                  children: [
                    _quickStartChip('Practice outcome', 'outcome', 'personal_goal', 'What changed after the rep?'),
                    _quickStartChip('Feedback quote', 'feedback', 'job', 'Who said it, and what did it prove?'),
                    _quickStartChip('Public artifact', 'artifact', 'portfolio', 'Add the link, filename, or screenshot note.'),
                  ],
                ),
                const SizedBox(height: 14),
                ...
                const SizedBox(height: 14),
                Text('Place this could help', style: _labelStyle()),
                const SizedBox(height: 5),
                Text(
                  'Choose where this proof might matter later. Echo uses this to find missing evidence.',
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _opportunityChoice('portfolio', 'Portfolio'),
                    ...
                    _opportunityChoice('community', 'Community'),
                  ],
                ),
                const SizedBox(height: 18),
                SizedBox(
  ...
  Widget _choice(String value, String label) {
    ...
    return GestureDetector(onTap: () => setState(() => _category = value), child: _chip(label, selected));
  }

  Widget _opportunityChoice(String value, String label) {
    final selected = _opportunityType == value;
    return GestureDetector(onTap: () => setState(() => _opportunityType = value), child: _chip(label, selected));
  }

  Widget _quickStartChip(String label, String category, String opportunityType, String evidenceHint) {
    return GestureDetector(
      onTap: () {
        setState(() {
          _category = category;
          _opportunityType = opportunityType;
          if (_evidenceCtrl.text.trim().isEmpty) _evidenceCtrl.text = evidenceHint;
        });
      },
      child: _chip(label, _category == category && _opportunityType == opportunityType),
    );
  }
