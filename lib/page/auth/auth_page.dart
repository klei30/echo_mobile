...
class _AuthPageState extends State<AuthPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _login() async {
    ...
    final err = await AuthService().login(email: _loginEmail.text.trim(), password: _loginPassword.text);
    if (!mounted) return;
  ...
  Future<void> _register() async {
    ...
    final err = await AuthService().register(email: _regEmail.text.trim(), username: _regUsername.text.trim(), password: _regPassword.text);
    if (!mounted) return;
    setState(() => _loading = false);
    if (err != null) {
      setState(() => _error = err);
  ...
  Future<void> _openHomeBrainUrl() async {
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: ClipRRect(borderRadius: BorderRadius.circular(18), child: Image.asset('assets/echo_logo.png', width: 56, height: 56)),
                      ),
                      const SizedBox(height: 16),
                      Text(
                        'Echo',
                        style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: EchoColors.textPrimary),
                      ),
                      Text('Your personal learning system', style: TextStyle(fontSize: 14, color: EchoColors.textGhost)),
                            ...
                            children: [
                              ...
                              TabBar(
                                ...
                                tabs: [
                                  ...
                                  Tab(text: compact ? 'Create' : 'Create Account'),
                                ],
                              ),
                              const SizedBox(height: 24),
                              ...
                              SizedBox(
                                ...
                                child: TabBarView(controller: _tabs, children: [_buildLogin(), _buildRegister()]),
                              ),
                                ...
                                Container(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      Expanded(
                                        child: Text(_error!, style: TextStyle(color: Color(0xFFE08080), fontSize: 13)),
                                      ),
                                    ],
                                  ),
                                ),
                              ...
                              SizedBox(
                                ...
                                height: 48,
                                child: FilledButton(
                                  style: FilledButton.styleFrom(backgroundColor: EchoColors.amber, foregroundColor: Colors.white),
                      ...
                      const SizedBox(height: 24),
                      Text('Your data stays on your device.', style: TextStyle(fontSize: 12, color: EchoColors.textGhost)),
                    ],
                  ),
                ),
              ),
  ...
  Widget _connectionPill() {
    ...
    final current = host.resolvedUrl;
    final connected = host.hasTunnel || current.contains('172.') || current.contains('192.168.');
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            connected ? Icons.computer_rounded : Icons.settings_ethernet_rounded,
            size: 16,
            color: connected ? EchoColors.amber : EchoColors.textGhost,
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  connected ? 'Home Brain URL set' : 'Choose Home Brain URL',
                  style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: EchoColors.textPrimary),
                ),
                Text(
                  current,
                  maxLines: 1,
                  overflow: TextOverflow.ellipsis,
                  style: TextStyle(fontSize: 10.5, color: EchoColors.textGhost),
                ),
              ],
            ),
          ),
          IconButton(
            onPressed: _openHomeBrainUrl,
            tooltip: 'Edit Home Brain URL',
  ...
  Widget _buildRegister() {
    return Column(
      children: [
        _field(controller: _regEmail, label: 'Email', icon: Icons.email_outlined, keyboardType: TextInputType.emailAddress),
        const SizedBox(height: 10),
        _field(controller: _regUsername, label: 'Username', icon: Icons.person_outlined),
        const SizedBox(height: 10),
        _field(
        ...
        const SizedBox(height: 10),
        _field(controller: _regConfirm, label: 'Confirm Password', icon: Icons.lock_outlined, obscure: _obscureReg, onSubmit: (_) => _register()),
      ],
    );
  }
