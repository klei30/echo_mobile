...
class _ChatPageState extends State<ChatPage> {
  ...
  List<String> _buildPersonalizedStarters(EchoLoopState loop) {
    ...
    if (thesis != null) {
      ...
      if (statement != null && statement.isNotEmpty && statement != 'Echo is waiting for enough real moments to form a thesis.') {
        starters.add('My current read: "$statement" - push back on this.');
      }
    }
  ...
  Widget _buildLoopContext(EchoLoopState loop) {
    final thesis = loop.thesis;
    final confidence = thesis?['confidence_label'] as String?;
    final priority = loop.todayPriority;
    final priorityTitle = priority?['title'] as String?;
    final label = confidence != null && confidence.isNotEmpty
        ? 'Current read: $confidence'
        : priorityTitle != null && priorityTitle.isNotEmpty
        ? 'Today: $priorityTitle'
        : null;
    if (label != null) {
      return Row(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            Icon(Icons.arrow_forward_rounded, size: 15, color: EchoColors.textGhost),
          ],
        ),
      ),
    );
  ...
  _StarterLabel _displayStarterLabel(String raw) {
    if (raw.startsWith('My current thesis:') || raw.startsWith('My current read:')) {
  ...
  String _buildDeviceEchoInjection(EchoLoopState loop) {
    ...
    final buffer = StringBuffer()
      ...
      ..writeln('Use this loop: Discover direction -> Practice today -> Produce proof -> Log outcome -> Update Echo -> Unlock opportunity.')
      ..writeln('Be direct, calm, evidence-based, and focused on one concrete next step, one better decision, or one piece of proof.')
      ..writeln('You have no live internet, no cloud actions, and no connected apps in this mode.')
    ...
    buffer.writeln(
      '\nIf cached context is thin, ask one useful question or give one concrete next step. Avoid anime-style language. Use "Practice Versions" for the Echo feature that compares several approaches to a hard choice or skill.',
    );
  ...
  Widget _buildContextRow() {
    ...
    final todayLabel = (loop.todayPriority?['title'] as String?)?.split(' ').take(3).join(' ') ?? 'next step';
    final readLabel = (loop.thesis?['title'] as String?)?.split(' ').take(4).join(' ') ?? 'forming';
    return Padding(
      padding: const EdgeInsets.fromLTRB(12, 2, 12, 4),
      child: Row(
        children: [
          _contextMini(Icons.flag_outlined, 'Today', todayLabel, EchoColors.amber),
          const SizedBox(width: 6),
          _contextMini(Icons.psychology_alt_outlined, 'Read', readLabel, EchoColors.proof),
          const SizedBox(width: 6),
          _contextMini(Icons.hub_outlined, 'Mode', runtime.modeLabel, EchoColors.memory),
        ],
      ),
    );
  }

  Widget _contextMini(IconData icon, String label, String value, Color accentColor) {
    return Expanded(
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 9, vertical: 8),
        decoration: BoxDecoration(
          color: Color.alphaBlend(accentColor.withValues(alpha: 0.05), EchoColors.surface),
          borderRadius: BorderRadius.circular(14),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(
                    label.toUpperCase(),
                    style: GoogleFonts.spaceMono(fontSize: 7.4, fontWeight: FontWeight.w700, color: accentColor, letterSpacing: 0.35),
                  ),
                  ...
                  Text(
                    ...
                    maxLines: 1,
                    overflow: TextOverflow.ellipsis,
                    style: GoogleFonts.plusJakartaSans(fontSize: 10.2, fontWeight: FontWeight.w700, color: EchoColors.textPrimary),
                  ),
                ],
              ),
            ),
