...
void _handleDeepLink(Uri uri) {
  if (uri.scheme != 'echo') return;
  // echo://pair?baseUrl=https%3A%2F%2F...
  final baseUrl = uri.queryParameters['baseUrl'] ?? uri.queryParameters['url'] ?? uri.queryParameters['tunnelUrl'];
  if (baseUrl == null || baseUrl.isEmpty) return;
  // Delay slightly to ensure navigator is mounted
  Future.delayed(const Duration(milliseconds: 400), () {
    navigatorKey.currentState?.push(MaterialPageRoute(builder: (_) => PairComputerScreen(initialUrl: baseUrl)));
  });
}
  ...
  Widget build(BuildContext context) {
    return Consumer<SettingsProvider>(
      builder: (context, settings, child) {
        final theme = settings.generalSetting.theme;
        ...
        return MaterialApp(
          ...
          darkTheme: ThemeData(
            ...
            fontFamily: getPlatformFontFamily(),
            colorScheme: ColorScheme.fromSeed(seedColor: const Color(0xFF83BDF2), brightness: Brightness.dark),
          ),
...
class AuthGate extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AuthPage(
      onAuthenticated: () {
        Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (_) => const LayoutPage()));
      },
    );
  }
}
