...
class OAuthDiscoveryResult {
  ...
  const OAuthDiscoveryResult({required this.requiresOAuth, this.authorizationUrl, this.tokenUrl, this.clientId, this.scope, this.redirectUri});
}
...
class OAuthDiscoveryService {
  ...
  static Future<OAuthDiscoveryResult> _tryWellKnownEndpoint(String baseUrl) async {
    try {
      ...
      final response = await http.get(Uri.parse(wellKnownUrl), headers: {'Accept': 'application/json'}).timeout(const Duration(seconds: 5));
  ...
  static Future<OAuthDiscoveryResult> _tryDirectServerProbe(String serverUrl) async {
    try {
      ...
      if (serverUrl.contains('/sse') || serverUrl.contains('/mcp')) {
        final response = await http.get(Uri.parse(serverUrl), headers: {'Accept': 'text/event-stream'}).timeout(const Duration(seconds: 5));
  ...
  static Future<OAuthDiscoveryResult> _tryOAuthMetadataEndpoint(String baseUrl) async {
    try {
      ...
      final response = await http.get(Uri.parse(metadataUrl), headers: {'Accept': 'application/json'}).timeout(const Duration(seconds: 5));
