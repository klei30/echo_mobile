...
class EchoApiClient {
  ...
  Future<String> getTrainingStatus({String? lane}) async {
    try {
      final uri = Uri.parse('$_base/v1/training/status').replace(queryParameters: lane == null ? null : {'lane': lane});
  ...
  Future<Map<String, dynamic>?> getTrainingSummary({String? lane}) async {
    try {
      final uri = Uri.parse('$_base/v1/training/summary').replace(queryParameters: lane == null ? null : {'lane': lane});
  ...
  Future<List<Map<String, dynamic>>> getTrainingRuns({String? lane}) async {
    try {
      final uri = Uri.parse('$_base/v1/training/runs').replace(queryParameters: lane == null ? null : {'lane': lane});
      final resp = await http.get(uri, headers: _h).timeout(const Duration(seconds: 10));
      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body) as Map<String, dynamic>;
  ...
  Future<Map<String, dynamic>?> getTrainingEval({String? lane}) async {
    try {
      final uri = Uri.parse('$_base/v1/training/eval').replace(queryParameters: lane == null ? null : {'lane': lane});
      final resp = await http.get(uri, headers: _h).timeout(const Duration(seconds: 10));
      if (resp.statusCode == 200) return jsonDecode(resp.body) as Map<String, dynamic>;
  ...
  Future<Map<String, dynamic>?> getRuntimeCapabilities() async {
    try {
      final resp = await http.get(Uri.parse('$_base/v1/runtime/capabilities'), headers: _h).timeout(const Duration(seconds: 8));
      if (resp.statusCode == 200) return jsonDecode(resp.body) as Map<String, dynamic>;
      _log.warning('getRuntimeCapabilities HTTP ${resp.statusCode}');
    } catch (e) {
      _log.warning('getRuntimeCapabilities error: $e');
    }
    return null;
  }

  Future<bool> proposeMemory({required String text, String sourceType = 'talk', String privacy = 'private'}) async {
    final trimmed = text.trim();
    if (trimmed.isEmpty) return false;
    try {
      final resp = await http
          .post(
            Uri.parse('$_base/v1/memory/propose'),
            headers: {..._h, 'Content-Type': 'application/json'},
            body: jsonEncode({'text': trimmed, 'source_type': sourceType, 'privacy': privacy}),
          )
          .timeout(const Duration(seconds: 10));
      if (resp.statusCode == 200) return true;
      _log.warning('proposeMemory HTTP ${resp.statusCode}');
    } catch (e) {
      _log.warning('proposeMemory error: $e');
    }

    // Older Echo backends may not have the consent endpoint yet. Preserve the
    // user's intent as an outcome so the lesson can still be reviewed later.
    return recordOutcome(
      subjectType: 'memory_proposal',
      outcome: 'memory_requested',
      score: 0.55,
      note: jsonEncode({'text': trimmed, 'source_type': sourceType, 'privacy': privacy}),
    );
  }
  ...
  Future<Map<String, dynamic>?> recordOutcomeJson({
    ...
    try {
      final resp = await http
          ...
          .timeout(const Duration(seconds: 10));
      if (resp.statusCode == 200) return jsonDecode(resp.body) as Map<String, dynamic>;
      _log.warning('recordOutcome HTTP ${resp.statusCode}');
    } catch (e) {
  ...
  Future<bool> recordOutcome({required String subjectType, String? subjectId, required String outcome, double score = 0.5, String note = ''}) async {
    final result = await recordOutcomeJson(subjectType: subjectType, subjectId: subjectId, outcome: outcome, score: score, note: note);
    return result != null;
  }
  ...
  Future<Map<String, dynamic>?> triggerTraining({String? lane}) async {
    try {
      ...
      final resp = await http.post(Uri.parse('$_base/trigger-training'), headers: h, body: jsonEncode(body)).timeout(const Duration(seconds: 10));
      if (resp.statusCode == 200) return jsonDecode(resp.body) as Map<String, dynamic>;
  ...
  Future<List<Map<String, dynamic>>> getTrainingHistory({String? lane}) async {
    try {
      final uri = Uri.parse('$_base/v1/training/history').replace(queryParameters: lane == null ? null : {'lane': lane});
      final resp = await http.get(uri, headers: _h).timeout(const Duration(seconds: 10));
      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body) as Map<String, dynamic>;
  ...
  /// Cold-start stage for TODAY tab variants: day0 / early / building / active
  Future<Map<String, dynamic>?> getOnboardingState() async {
    try {
      final resp = await http.get(Uri.parse('$_base/v1/user/onboarding-state'), headers: _h).timeout(const Duration(seconds: 10));
      if (resp.statusCode == 200) return jsonDecode(resp.body) as Map<String, dynamic>;
      _log.warning('getOnboardingState HTTP ${resp.statusCode}');
    } catch (e) {
      _log.warning('getOnboardingState error: $e');
    }
    return null;
  }

  /// Unified after-reply action strip.
  /// action: make_practice | save_memory | log_outcome | add_to_passport
  Future<Map<String, dynamic>?> replyAction({
    ...
    try {
      ...
      final resp = await http
          .post(
            Uri.parse('$_base/v1/reply/action'),
            headers: h,
            body: jsonEncode({
              'action': action,
          ...
          .timeout(const Duration(seconds: 12));
      if (resp.statusCode == 200) return jsonDecode(resp.body) as Map<String, dynamic>;
      _log.warning('replyAction HTTP ${resp.statusCode}');
    } catch (e) {
      _log.warning('replyAction error: $e');
    }
    return null;
  }

  /// Privacy-safe shareable Growth Card payload.
  Future<Map<String, dynamic>?> getGrowthCard() async {
    try {
      final resp = await http.get(Uri.parse('$_base/v1/passport/growth-card'), headers: _h).timeout(const Duration(seconds: 10));
      if (resp.statusCode == 200) return jsonDecode(resp.body) as Map<String, dynamic>;
      _log.warning('getGrowthCard HTTP ${resp.statusCode}');
    } catch (e) {
      _log.warning('getGrowthCard error: $e');
    }
    return null;
  }
